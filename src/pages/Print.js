...
// Helper function to generate barcode data URL without text
const generateBarcodeDataUrl = (value, width = 100, height = 30) => {
...
const Barcode = ({ value, width = 100, height = 30 }) => {
  const [barcodeDataUrl, setBarcodeDataUrl] = useState('');

  useEffect(() => {
    setBarcodeDataUrl(generateBarcodeDataUrl(value, width, height));
  }, [value, width, height]);
  ...
  const generatePrintHTML = (printLabels) => {
    ...
    for (let i = 0; i < printLabels.length; i += labelsPerPage) {
      ...
      const labelsHTML = pageLabels.map((label, index) => {
        ...
        const pricePerUnit = label.pricePerUnit ? `prix/${priceUnit} ${formatPrice(label.pricePerUnit)}€` : '';
        const quantity = displayUnit === 'pièce' && !label.unit ? label.quantity || '' : label.quantity && displayUnit ? `${formatPrice(label.quantity)}${displayUnit}` : label.quantity || '';
        const isLastInRow = (index + 1) % labelsPerRow === 0;
        const marginRight = isLastInRow ? '0px' : '10px';

        // Assurez-vous que barcodeValue est une seule valeur
        const barcodeValue = typeof label.barcode === 'string' ? label.barcode : (Array.isArray(label.barcode) ? label.barcode[0] : '');

        return `
          <div class="print-label" style="width: ${labelWidth}px; height: ${labelHeight}px; background-color: white; border: 0.3px dashed #000; border-radius: 0px; padding: 6px; font-size: 5px; margin-right: ${marginRight};">
            <div class="label-header" style="height: 45px;">
              <div class="product-name" style="font-size: 12px; font-weight: bold; color: #000; line-height: 1.2; height: 16px; overflow: hidden; text-align: left;">${truncateText(label.name, 20)}</div>
              <div class="price-frame" style="background-color: ${categoryColor}; border: 1px solid ${categoryColor}; border-top-left-radius: 60px; margin-left: 10px; width: 90%; padding: 0px; -webkit-print-color-adjust: exact; print-color-adjust: exact;">
                <div class="price-main" style="color: #000000; font-size: 14px; font-weight: bold; text-align: center; margin-left: 60px;">${formatPrice(label.price)}€</div>
                <div class="price-details" style="width: 90%; display: flex; justify-content: space-between; font-size: 7px; margin-top: 0.1px;">
                  <span class="price-unit" style="color: #000000">${pricePerUnit}</span>
                  <span class="quantity" style="color: #000000">${quantity}</span>
                </div>
              </div>
            </div>
            <div class="label-footer" style="height: 55px;">
              ${
                barcodeValue
                  ? `
                      <div class="barcode-section" style="height: 40px; display: flex; flex-direction: column; align-items: center; justify-content: flex-start; margin-top: 1px;">
                        <img src="${generateBarcodeDataUrl(barcodeValue, 130, 36)}" class="barcode-image" style="width: 100%; height: 25px; object-fit: contain;margin-top: 1px;" />
                        <div class="barcode-number" style="font-family: monospace; font-size: 11px; color: #000; line-height: 1; margin-top: 0px; height: 10px;">${barcodeValue}</div>
                      </div>
                    `
                  : '<div class="barcode-section" style="height: 40px;"></div>'
              }
            </div>
          </div>
        `;
      ...
      pages.push(`
        <div class="print-page">
          <div style="text-align: center; margin-bottom: 13px; font-size: 10px; color: #666;">
            <p>Impression de ${pageLabels.length} étiquettes (175 mm x 100 mm) - ${new Date().toLocaleDateString('fr-FR')}</p>
          </div>
      ...
      <head>
        ...
        <style>
          * { box-box-sizing: border-box; margin: 0; padding: 0; }
          ...
          .print-label { width: ${labelWidth}px; height: ${labelHeight}px; border: 0.3px dashed #000; border-radius: 0px; padding: 6px; margin: 0; display: inline-block; vertical-align: top; box-sizing: border-box; page-break-inside: avoid; position: relative; overflow: hidden; background-color: white; font-size: 5px; }
          .label-header { height: 45px; display: flex; flex-direction: column; margin-bottom: 8px; }
          .product-name { font-size: 12px; font-weight: bold; color: #000; line-height: 1.2; height: 16px; overflow: hidden; text-align: left; }
          ...
          .price-main { font-size: 14px; font-weight: bold; color: #000000; text-align: center; margin-left: 60px; }
          .price-details { width: 90%; display: flex; justify-content: space-between; font-size: 7px; margin-top: 0.1px; }
          .price-unit { text-align: left; color: #000000; }
          .quantity { text-align: right; color: #000000; }
          .label-footer { height: 55px; }
          .barcode-section { height: 40px; display: flex; flex-direction: column; align-items: center; justify-content: flex-start; margin-top: 8px; }
          .barcode-image { width: 100%; height: 36px; object-fit: contain; }
          .barcode-placeholder { height: 40px; }
          @media print {
                    ...
                    return (
                      ...
                      >
                        ...
                        <p style={{ margin: 0, fontSize: '14px', color: '#666' }}>
                          {cat.count} étiquettes disponibles
                        </p>
                      </div>
                    );
                  })}
                        ...
                        >
                          ...
                          <div style={{ flex: 1 }}>
                            ...
                            <p style={{ margin: 0, fontSize: '12px', color: '#666' }}>
                              {quantity} | {pricePerUnit} | {formatPrice(label.price)}€
                            </p>
                          </div>
                        </div>
                      );
                            ...
                            >
                              ...
                              <div style={{ flex: 1 }}>
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '4px' }}>
                                  ...
                                  <span style={{ fontWeight: 'bold', color: '#000000' }}>{formatPrice(label.price)}€</span>
                                </div>
            ...
            <div style={{ backgroundColor: 'white', padding: '20px', borderRadius: '8px' }}>
              ...
              >
                <div style={{ marginBottom: '15px', textAlign: 'center', fontSize: '12px', color: '#666' }}>
                  <p>Aperçu de {getLabelsForPrint().length} étiquette(s) - 175 mm x 100 mm, 3 par ligne</p>
                </div>
                <div
                  style={{
                    display: 'grid',
                  ...
                  {getLabelsForPrint().map((label) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        >
                          <div style={{ fontSize: '14px', fontWeight: 'bold', marginLeft: '60px', color: '#000000' }}>
                        ...
                        <div style={{ height: '55px', marginBottom: '4px' }}>
                          {label.barcode ? (
                            <div style={{ height: '40px', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'flex-start', marginTop: '3px' }}>
                              <Barcode value={label.barcode} width={100} height={24} />
                              <div style={{ fontFamily: 'monospace', fontSize: '10px', color: '#000', lineHeight: 1, height: '8px', display: 'flex', marginTop: '-2px', alignItems: 'center' }}>
                                {label.barcode}
                              </div>
                            </div>
